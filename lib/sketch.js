...
class Sketch {
  ...
  getLayerWithID(id) {
    return getLayerWithID(this.pages, id);
  }

  getLayersNamed(name) {
    return getLayersNamed(this.pages, name);
  }
...
function getLayerWithID(layers, id) {
  const tmp = layers.filter(layer => {
    if (!layer.id === id) {
      return getLayerWithID(layer, id);
    }
    return layer.id === id;
  });
...
function getLayersNamed(layers, name) {
  const tmp = layers.filter(layer => {
    if (!layer.name === name) {
      return getLayersNamed(layer, name);
    }
